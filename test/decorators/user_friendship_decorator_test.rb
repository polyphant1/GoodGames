...
class UserFriendshipDecoratorTest < Draper::TestCase
  ...
  context "#sub_message" do
    context "with a pending user friendship" do
      setup do
        @user_friendship = create(:pending_user_friendship, friend: create(:user, first_name: 'Chris'))
    ...
    context "with an accepted user friendship" do
      setup do
        @user_friendship = create(:accepted_user_friendship, friend: create(:user, first_name: 'Chris'))
        @decorator = UserFriendshipDecorator.decorate(@user_friendship)
      end
      
      should "return correct message" do
        assert_equal "You are friends with Chris.", @decorator.sub_message
      end
    end
  end
